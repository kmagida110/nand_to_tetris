...
class CodeWriter(object):
	...
	def __init__(self):
		...
		self.active_function = "" # Used this variable setup and propagation through code on advice from Xiaorui Tang
	...
	def get_program_flow_string(self,command_dict):
		if command_dict['command'] == 'label':
			...
			rv = self.function(command_dict['segment'], command_dict['index'])
		elif command_dict['command'] == 'return':
			rv = self.return_string()
		elif command_dict['command'] == 'call':
			rv = self.call(command_dict['segment'],command_dict['index'])

		return rv

	def call(self,function, n_args):
	...
	def return_string(self):
		# Place Frame in R13
		rv = '@LCL\nD=M\n@FRAME\nM=D\n'
		# Get and place return address in RET
		rv += '@5\nA=D-A\n'
		rv += 'D=M\n@RET\nM=D\n'
	...
	def function(self,function_name,num_locals):
		num_locals = int(num_locals)
		string = function_name
		self.active_function = string + '$'
		rv = '(' + string + ')\n'
		if num_locals < 1:
			return rv
		else:
	...
	def goto(self,segment):
		return '@' + self.active_function + str(segment) + '\n0;JMP\n'

	def if_goto(self,segment):
		return self.load_top_and_decrement() + '@' + self.active_function + str(segment) + '\nD;JEQ\n'
	...
	def eq(self):
		...
		# Set if false
		final_string += '@SP\nA=M-1\nM=0\n@CONTINUE' + str(self.label_counter) +'\n0;JMP\n'
		# Set if true
		final_string += '(TRUE' + str(self.label_counter) + ')\n@SP\nA=M-1\nM=-1\n(CONTINUE' + str(self.label_counter) +')\n'
		self.label_counter += 1
		return final_string
...
def translate_file(filename,code_writer):
	'''
	...
	for command_dict in parsed_list:		
		write_string += code_writer.run_command(command_dict)
...
if __name__ == '__main__':
	...
	with open(new_file,'w') as write_file:
		init_string = '@256\nD=A\n@SP\nM=D\n'
		cw = CodeWriter()
		init_string += cw.call('Sys.init',0)
		write_file.write(init_string)
		for filename in file_list:
			if '.vm' in filename:
				write_file.write(translate_file(filename,cw))
